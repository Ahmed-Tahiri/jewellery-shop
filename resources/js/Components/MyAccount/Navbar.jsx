...
let NavLink = ({ path, title }) => {
    ...
    return <li className="w-full"><Link href={path} className={`flex p-3 border-1 border-gray-300 shadow-xs w-full font-poppins text-lg ${activePath === path ? 'bg-mustard' : 'bg-white'} transition-colors ease-linear duration-200 hover:bg-mustard`}>{title}</Link></li>;
}
export let Navbar = () => {
    let navLinks = [
        {
