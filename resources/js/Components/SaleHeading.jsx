...
export let SaleHeading = () => {
    ...
    return (
        <div className={`w-full bg-zinc py-3 px-3 relative top-0 sm:px-6 md:px-10 lg:px-15 xl:px-20  items-center justify-center`}>
            <div className="w-full max-w-7xl mx-auto flex flex-row items-center justify-center sm:justify-between gap-x-3">
                <div className="text-white font-poppins text-base sm:block hidden"><p>Call Us: <span>+123-456-789</span></p></div>
                <div><h6 ref={heading} className="font-poppins text-white text-xs md:text-base flex md:flex-row flex-col items-center gap-1">{auth === null ? (<><span>Sign up and GET 25% OFF for your first order.</span> <Link href={'/signup'} className="underline text-mustard">Signup up now</Link></>) : <span className="text-center text-xs sm:text-base">{messages[index]}</span>}</h6></div>
