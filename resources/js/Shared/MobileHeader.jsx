...
export let MobileHeader = () => {
    ...
    let navIconsMobile = [
        { name: <GoHeart className='text-zinc text-xl' />, link: '#' },
        { name: <HiOutlineShoppingBag className='text-zinc text-xl' />, link: '#' },
        { name: <GoPerson className='text-zinc text-xl' />, link: '/myaccount' },
    ]
    return <>

        <div className={`w-8/10 fixed top-0 left-0 min-h-[630px] h-screen bg-white z-50 md:hidden ${navIsOpen ? 'flex' : 'hidden'} flex-col items-start justify-start  gap-y-0`}>
        ...
        <div className="w-full bg-transparent py-2 px-3 flex md:hidden items-center justify-between mb-3">
            <div className='flex items-center gap-x-2'>
                <div><button><AiOutlineMenu className='text-xl text-zinc' onClick={() => setNavIsOpen(true)} /></button></div>
                <div><button><RiSearch2Line className='text-zinc text-xl' /></button></div>
            </div>
