...
export let SubCategoryCard = ({ category, index }) => {
    ...
    const handleChange = (nextChecked) => {
        ...
        router.patch(route('admin.subcategories.status.update', category.id), { is_active: nextChecked ? 1 : 0 })
    };
    return <tr className={`${index % 2 === 0 ? "bg-powder-gray" : "bg-white"} hover:bg-gray-50 transition shadow`}>
        ...
        <td className="px-3 py-2 font-poppins font-normal text-base border-x-1 border-gray-300">{category.category.name}</td>
