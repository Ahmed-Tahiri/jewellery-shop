    ...
    return <div className="w-full group">
        <div className="flex flex-col items-start justify-start  w-full gap-y-2">
            ...
            <div className="border-gray-300 flex items-center justify-between w-full border-2  shadow-xs group-focus-within:border-light-gray group-focus-within:border-[3px] transition-all ease-linear duration-200">
                <input value={data} onChange={inputChangeHandler} type={`${passIsVisible ? 'text' : 'password'}`} name={name} id={id} className="font-poppins w-calc p-2 text-semi-black outline-0 border-0 text-sm sm:text-base" placeholder={placeholder} />
                <div className="w-7 me-1"><button type="button" className="cursor-pointer w-full flex items-center justify-center transition-all ease-linear duration-200" onClick={() => setPassIsVisible(!passIsVisible)}>{passIsVisible ? (<AiOutlineEyeInvisible className="text-zinc hover:text-light-gray text-2xl" />) : (<AiOutlineEye className="text-zinc hover:text-light-gray text-2xl" />)}</button></div>
            </div>
