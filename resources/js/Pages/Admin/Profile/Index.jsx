...
let AdminDataField = ({ label, value }) => {
    return <div className="flex flex-col gapy-2 items-start">
        <h4 className="font-poppins md:text-base text-sm  font-normal">{label}</h4>
        <p className="font-poppins text-light-gray md:text-base text-sm font-medium">{value}</p>
    </div>
}
        ...
        <section className="w-full min-h-170 ">
            <div className="w-full flex flex-col gap-y-8">
                ...
                <div className="flex flex-col w-full bg-white shadow rounded gap-y-5 p-5">
                    <div className={`w-full flex flex-row gap-x-5  items-center ${canEdit ? 'justify-end md:justify-between' : 'justify-between'} border-b-[1px] border-gray-300 pb-4`}>
                        <h4 className={`font-poppins text-lg font-medium text-semi-black ${canEdit ? 'hidden md:block' : 'block'}`}>Personal Information</h4>
                        {canEdit ? <div className="flex flex-row gap-x-3 w-50">
                            <button type="reset" form="adminUpdateForm" onClick={editCancelHandle} className="md:text-base text-sm shadow px-4 py-2 bg-light-gray text-white font-poppins cursor-pointer flex-1 transition-colors ease-linear duration-200 hover:bg-dark-gray"> Cancel </button>
                            <button onClick={editSaveHandle} className={`md:text-base text-sm shadow px-4 py-2 bg-mustard text-white font-poppins cursor-pointer flex-1 transition-colors ease-linear duration-200 hover:bg-mustard-dark ${processing ? 'opacity-50' : 'opacity-100'}`} form="adminUpdateForm" type="submit" disabled={processing}> {processing ? 'Saving...' :
                                'Save'} </button>
                        </div> :
                            <button className="flex flex-row gap-x-2 items-center p-2 bg-mustard text-white shadow  cursor-pointer transition-colors ease-linear duration-200 hover:bg-mustard-dark" onClick={() => setCanEdit(true)}>
                                <span className="font-poppins font-medium md:text-base text-sm">Edit</span>
